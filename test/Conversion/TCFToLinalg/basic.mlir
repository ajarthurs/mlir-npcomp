...
// CHECK-SAME:                     %[[FILTER:.*]]: tensor<?x?xf32>) -> tensor<?x?xf32> {
// CHECK:           %[[C0F32:.*]] = constant 0.000000e+00 : f32
// CHECK:           %[[C0:.*]] = constant 0 : index
// CHECK:           %[[C1:.*]] = constant 1 : index
// CHECK:           %[[INK:.*]] = dim %[[IN]], %[[C1]] : tensor<?x?xf32>
// CHECK:           %[[FILTERK:.*]] = dim %[[FILTER]], %[[C0]] : tensor<?x?xf32>
...
// CHECK:           %[[RET:.*]] = shape.assuming %[[WINESS]] -> (tensor<?x?xf32>) {
// CHECK:             %[[INROWS:.*]] = dim %[[IN]], %[[C0]] : tensor<?x?xf32>
...
// CHECK:           }
// CHECK:           return %[[RET:.*]] : tensor<?x?xf32>
func @tcf_conv_2d(%arg0: tensor<?x?xf32>, %arg1: tensor<?x?xf32>) -> tensor<?x?xf32> {
  %0 = tcf.conv_2d %arg0, %arg1 : (tensor<?x?xf32>, tensor<?x?xf32>) -> tensor<?x?xf32>
  return %0 : tensor<?x?xf32>
}
